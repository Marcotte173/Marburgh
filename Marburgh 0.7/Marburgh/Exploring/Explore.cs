...
class Explore
{
    ...
    public static void ExploreNextRoom(Dungeon d, Creature p)
    {
        Console.Write("\n\nExploring");
        Utilities.DotDotDot();
        ...
        else if (dungeonRoomTally == d.rooms && d.boss.IsAlive == false) //This is when you hit last room but boss is dead. You get treasure but less.
        {
            ...
            Console.Clear();
            Console.WriteLine($"You make it to the " +Colour.BOSS + d.boss.name +Colour.RESET + "'s lair. He is long dead but you manage to find some treasure you didn't notice in your last pass");
            Console.WriteLine($"You gain " + Colour.GOLD + GoldGainInt + Colour.RESET + " gold");
            p.gold += GoldGainInt;
        ...
        Utilities.Keypress();
        Console.Clear();
        //Special room or regular room
        if (d.roomOptions[roomRoll].EventArray[0].eventType > 0) SpecialRoom(d, p, d.roomOptions[roomRoll].EventArray);
        else RegularRoom(d, p, roomRoll);
        GameDungeon(d,p);
    }
    ...
    public static void RegularRoom(Dungeon d, Creature p, int roomRoll)
    {
        //Check for monsters
    ...
    public static void MonsterSummon(Room room, Dungeon d, Creature p)
    {
        //Did monsters get summoned?
        int x = Utilities.rand.Next(1, 101);
        //If yes,
        int monsterChance = room.modifier + d.monsterSummon;
        if (x <= monsterChance && fought == false|| p.force)//Make monsters, fight them        
        {
            fought = true;
    ...
    public static void ExploreRoomSearch(Room room, Dungeon d, Creature p)
    {
        ...
        if (choice == "s")
        {
            ...
            for (int i = 0; i < EventDisplay.Count; i++)
            {
                ...
                if (EventDisplay[i].flavor == "Some" + Colour.GOLD + " gold" + Colour.RESET)
                {
                    ...
                    p.gold += room.EventArray[0].effect + (goldAdd * 6);
                    Utilities.EmbedColourText(Colour.GOLD, "You gain ", $"{room.EventArray[0].effect + (goldAdd * 6)}", " gold");
                    Thread.Sleep(500);
                }
                ...
                if (EventDisplay[i].flavor == "An old " + Colour.XP + "book" + Colour.RESET)
                {
                    ...
                    Console.WriteLine($"You gain " + Colour.XP + $"{ room.EventArray[0].effect + XPAdd} " + Colour.RESET + "experience");
                    Thread.Sleep(500);
                }
            }
            Utilities.Keypress();
        }
        else ExploreRoomSearch(room, d, p);
    }
}